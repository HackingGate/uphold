# ADR 0009: who asks whether a hook pin is current
...
needs an ordering of tags, and `version_key` in `src/pins.rs` is a hand-rolled
one. It shipped one bug already: `v0.11.0.1-1` sorted under `v0.11.0.1`, so
...
This record splits the question along that line, and keeps the probes that
decided it.
...
the same shape. Either is a fine companion to the hook, not a replacement.
