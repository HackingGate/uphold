...
significant of the differences.
...
"can one rule target equivalent constructs across several grammars" is no,
with a real mitigation: N rules in one schema, reviewed together, are better
than N implementations in N languages.
...
watch the helper they are required to go through — which is the half that
fails silently.
...
still in the file. The state is recoverable — a second rule matching
...
a time, and it is the most useful finding of this evaluation.
