...
fn run() -> Result<Exit> {
    ...
    match text_of(first)? {
        ...
        "supply-chain" => {
            ...
            // Loaded for three facts: whether it inherits the set that asks for
            // gitleaks, the guarddog findings it has waived, and who owns this
            // repository, which decides the first-party git dependencies. A
            // policy that will not load is exit 2 here as it is
            ...
            supply::run(&root, &scope, secrets, &loaded)
        }
        "probe" => {
            let usage = || {
                Fatal::new(format!(
...
fn guard_command(arguments: &[OsString]) -> Result<Exit> {
    ...
    if let Some(source) = text_source {
        ...
        // Everything this seam judges text by, from the one place that knows
        // what that is -- the guards, and the prose rules standing in front of
        // a command. A rule that refuses a sentence shape in a pull-request
        // body has nothing different to say about a release note piped in here,
        // and a seam that consulted one and not the other would be the same
        // rule with two answers.
        //
        // Both arms become a refusal, because that is the word this command
        ...
        let refusals: Vec<guard::Refusal> =
            text::judged(text::Seam::Guard, &root, &policy, &source, &text)?
                ...
                .map(|verdict| match verdict {
                    ...
                    text::Verdict::Rule(failure) => guard::Refusal {
