...
Bump `version` in `Cargo.toml`, rename the `## Unreleased` heading in
`CHANGELOG.md` to the new version and open a fresh `## Unreleased` above it,
merge, then:
...
and the GitHub Release, with the changelog section for that version as the
notes. Edit the workflow through `dist-workspace.toml` and `dist generate`,
